...
每个技能现在都有独立的详情页，包含完整的 SKILL.md 介绍、来源链接、所属分类与安装方式，可在顶部导航的「技能库」Tab 中浏览。其中 7 个**核心推荐技能**（`nec-resource-publisher`、`nec-wechat-sync`、`pv-station-rating`、`pv-storage-bom` 4 个仓库独家自研 + `patent-application-cn`、`cit-thesis-writer`、`acad-paper-prompter` 3 个学术精选）在索引与导航中前置展示，其余 143 个收录自各平台的推荐技能按 12 大分类组织。
...
<Card
  ...
  horizontal
>
  浏览全部技能的独立详情页（核心推荐前置 + 推荐技能按 12 大分类组织）
</Card>
