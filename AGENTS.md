...
7. **技能库** — Skill 分支 150 个 AI Coding Skills 的详情页（`skills/*.mdx`，由脚本从 `github/Skill` 分支批量生成）
   - 概览：`skills/index`（13 大类完整索引表）置顶
   - 13 个分组：飞书/Lark 办公套件（25）→ AI CLI 调用器（6）→ 前端 & 设计开发（14）→ 开发工程 & 工作流（17）→ 搜索 & 资讯聚合（8）→ 文档 & 办公文件（9）→ 集成 & 平台连接（14）→ 学术 & 论文（6）→ 新能源 & 光伏（2）→ QClaw/OpenClaw 平台（15）→ 效率 & 头脑风暴（9）→ 社交 & 微信生态（8）→ TRAE 内置 & 独立工具（17）
   - 分组顺序沿用 `community/skill-branch` 的 13 大类；分组内技能按索引表固有顺序排列，每个技能只归一类（如 `github-skill` 只放「开发工程 & 工作流」）
