---
title: "mintlify"
description: "Build and maintain documentation sites with Mintlify. Use when creating docs pages, configuring navigation, adding components, or setting up API refe…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/mintlify/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/mintlify/SKILL.md) |
| 分类 | 文档 & 办公文件 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/mintlify/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
