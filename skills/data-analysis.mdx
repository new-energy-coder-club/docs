---
title: "data-analysis"
description: "Use this skill when the user uploads Excel (.xlsx/.xls) or CSV files and wants to perform data analysis, generate statistics, create summaries, pivot…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/data-analysis/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/data-analysis/SKILL.md) |
| 分类 | TRAE 内置 & 独立工具 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/data-analysis/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
