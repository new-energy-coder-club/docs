---
title: "acad-paper-prompter"
description: "Academic paper writing assistant providing structured prompts for polishing, translating, discussion writing, response letters, cover letters, litera…"
---

> ⭐ **核心推荐**：本技能为 NEC 社区重点推荐技能，在[技能库索引](/skills/index)中前置展示。

| 项目 | 内容 |
|---|---|
| 来源 | [skills/acad-paper-prompter/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/acad-paper-prompter/SKILL.md) |
| 分类 | 核心推荐（NEC 精选） |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/acad-paper-prompter/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
