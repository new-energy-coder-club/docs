---
title: "neodata-financial-search"
description: "自然语言通用金融数据搜索服务。用自然语言查询股票、基金、指数、板块、宏观经济、外汇、大宗商品等全品类金融数据，涵盖行情报价、财务报表（财报）、资金流向、研报评级、事件公告等。"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/neodata-financial-search/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/neodata-financial-search/SKILL.md) |
| 分类 | 搜索 & 资讯聚合 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/neodata-financial-search/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
...
详细的数据服务目录和完整的出入参字段说明见 [reference.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/neodata-financial-search/reference.md)。
