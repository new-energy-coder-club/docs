...
description: "Redis performance optimization and best practices. Use this skill when working with Redis data structures, Redis Query Engine (RQE), vector search wi…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/redis-development/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/redis-development/SKILL.md) |
| 分类 | TRAE 内置 & 独立工具 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/redis-development/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
