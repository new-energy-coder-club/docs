---
title: "news-summary"
description: "This skill should be used when the user asks for news updates, daily briefings, or what's happening in the world. Fetches news from trusted internati…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/news-summary/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/news-summary/SKILL.md) |
| 分类 | 搜索 & 资讯聚合 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/news-summary/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
