---
title: "wechat-cli-export"
description: "Export WeChat group member lists via wechat-cli with automatic Windows encoding fixes. Use when the user needs to extract微信群成员列表, export wechat group…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/wechat-cli-export/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/wechat-cli-export/SKILL.md) |
| 分类 | 社交 & 微信生态 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/wechat-cli-export/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
