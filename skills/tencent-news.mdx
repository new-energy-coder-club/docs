...
| 项目 | 内容 |
|---|---|
| 来源 | [skills/tencent-news/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/SKILL.md) |
| 分类 | 集成 & 平台连接 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/tencent-news/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
...
按照 [`references/installation-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/installation-guide.md) 中的安装命令执行安装：

安装成功后重新执行 `sh scripts/cli-state.sh`（Windows 用 `bun scripts/cli-state.ts`）刷新状态。

若安装失败，参考 [`references/installation-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/installation-guide.md) 中的故障排查部分，引导用户手动处理。
...
若 `update` 命令失败，或错误信息表明当前 CLI 不支持 `update`（如 `unknown command`、`not found`、`not recognized`），按上述步骤 2 重新安装。仍然失败时，引导用户参考 [`references/update-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/update-guide.md) 手动处理。
...
详见 [`references/env-setup-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/env-setup-guide.md)。
...
- Token 自动获取说明：[`SETUP_TOKEN.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/SETUP_TOKEN.md)
- 用户手动安装指南：[`references/installation-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/installation-guide.md)
- 用户手动更新指南：[`references/update-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/update-guide.md)
- API Key 获取与手动配置：[`references/env-setup-guide.md`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/tencent-news/references/env-setup-guide.md)
