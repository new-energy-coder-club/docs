---
title: "lark-drive"
description: "飞书云空间：管理云空间中的文件和文件夹。上传和下载文件、创建文件夹、复制/移动/删除文件、查看文件元数据、管理文档评论、管理文档权限、订阅用户评论变更事件、修改文件标题（docx、sheet、bitable、file、folder、wiki）；也负责把本地 Word/Markdown/Excel/…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/lark-drive/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/SKILL.md) |
| 分类 | 飞书/Lark 办公套件 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/lark-drive/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
...
- 用户要在 Drive 里上传、创建、读取、覆盖更新**原生 `.md` 文件**（不是导入成 docx），切到 [`lark-markdown`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-markdown/SKILL.md)。
...
#### Reaction / 表情场景
- 遇到评论 / 回复上的 reaction（表情、各表情数量、谁点了什么、添加/删除表情）相关问题时，**先阅读 [lark-drive-reactions.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/skills/lark-drive/references/lark-drive-reactions.md) 了解如何使用**。
...
| Shortcut | 说明 |
|----------|------|
| [`+search`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-search.md) | Search Lark docs, Wiki, and spreadsheet files with flat filter flags (preferred over `docs +search`). Natural-language-friendly: `--edited-since`, `--mine`, `--doc-types`, etc. |
| [`+upload`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-upload.md) | Upload a local file to a Drive folder or wiki node |
| [`+create-folder`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-create-folder.md) | Create a Drive folder, optionally under a parent folder, with bot auto-grant support |
| [`+download`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-download.md) | Download a file from Drive to local |
| [`+status`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-status.md) | Compare a local directory with a Drive folder by SHA-256 content hash; reports `new_local` / `new_remote` / `modified` / `unchanged` (read-only diff primitive for sync workflows). Duplicate remote `rel_path` conflicts fail fast with `error.type=duplicate_remote_path` and list every conflicting entry; do not proceed as if one was chosen. `--local-dir` 必须是 cwd 内的相对路径，越界路径 CLI 会直接拒绝；目标在 cwd 外时引导用户切换 agent 工作目录，不要私自 `cd` 绕过。 |
| [`+pull`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-pull.md) | One-way **file-level** mirror of a Drive folder onto a local directory (Drive → local). Duplicate remote `rel_path` conflicts fail by default before writing; for duplicate files only, `--on-duplicate-remote rename` downloads all copies with stable hashed suffixes, while `newest` / `oldest` explicitly choose one. Supports `--if-exists` (overwrite/skip) and `--delete-local` for orphan cleanup; the destructive `--delete-local` requires `--yes` and only unlinks regular files — empty local directories left behind by remote folder deletes are NOT pruned. Item-level failures exit non-zero (`error.type=partial_failure`) and skip the `--delete-local` pass to avoid half-synced state. `--local-dir` is bounded to cwd by CLI path validation; tell the user to switch the agent's working directory if the target is outside cwd. |
| [`+create-shortcut`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-create-shortcut.md) | Create a shortcut to an existing Drive file in another folder |
| [`+add-comment`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-add-comment.md) | Add a comment to doc/docx/sheet/slides, also supports wiki URL resolving to doc/docx/sheet/slides |
| [`+export`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-export.md) | Export a doc/docx/sheet/bitable to a local file with limited polling; supports `--file-name` for local naming |
| [`+export-download`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-export-download.md) | Download an exported file by file_token |
| [`+import`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-import.md) | Import a local file to Drive as a cloud document (docx, sheet, bitable) |
| [`+move`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-move.md) | Move a file or folder to another location in Drive |
| [`+delete`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-delete.md) | Delete a Drive file or folder with limited polling for folder deletes |
| [`+push`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-push.md) | Mirror a local directory onto a Drive folder (local → Drive). Duplicate remote `rel_path` conflicts fail by default before upload / overwrite / delete; use `--on-duplicate-remote newest\|oldest` only when the conflict is duplicate files and you explicitly want to target one existing remote file. Supports `--if-exists` (overwrite/skip) and `--delete-remote` for one-way mirror sync; the destructive `--delete-remote` requires `--yes`. `--local-dir` is bounded to cwd by CLI path validation; tell the user to switch the agent's working directory if the source is outside cwd. |
| [`+task_result`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-task-result.md) | Poll async task result for import, export, move, or delete operations |
| [`+apply-permission`](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/lark-drive/references/lark-drive-apply-permission.md) | Apply to the document owner for view/edit access (user-only; 5/day per document) |
