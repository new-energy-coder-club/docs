---
title: "nec-resource-publisher"
description: "将培训/资料附件（PDF、Excel、工程图、压缩包等）发布到 NEC 文档站（new-energy-coder-club/docs，Mintlify）——原文原图展示。PDF 逐块提取原文并原位插入插图，矢量图页与乱码页整页渲染为高清图片，Excel 全量转 Markdown 表格，所有文件与图…"
---

> 🌟 **仓库独家 · 核心推荐**：本技能由 NEC 社区自研，仅托管于本仓库 Skill 分支，在[技能库索引](/skills/index)的核心推荐板块中前置展示。

| 项目 | 内容 |
|---|---|
| 来源 | [skills/nec-resource-publisher/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/nec-resource-publisher/SKILL.md) |
| 分类 | 核心推荐（仓库独家） |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/nec-resource-publisher/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
...
- 每个 Sheet 转一张 Markdown 表：自动裁掉全空列、空表头补默认名、单元格换行转 `<br/>`、纯 URL 单元格转 ``打开链接``。
