...
| 项目 | 内容 |
|---|---|
| 来源 | [skills/pdf/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/pdf/SKILL.md) |
| 分类 | 文档 & 办公文件 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/pdf/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
