---
title: "qclaw-env"
description: "OpenClaw skill 全链路环境诊断与安装工具。安装任何 CLI、命令行工具、包管理器、运行时环境时必须使用此 skill。使用场景包括但不限于：(1) 安装或配置任何命令行工具或 CLI（如 gh、ffmpeg、whisper 等），(2) 安装 OpenClaw skill 所需的依赖…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/qclaw-env/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/qclaw-env/SKILL.md) |
| 分类 | QClaw/OpenClaw 平台 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/qclaw-env/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
