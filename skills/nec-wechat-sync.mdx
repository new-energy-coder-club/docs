---
title: "nec-wechat-sync"
description: "| 同步分析 NEC 战队微信群聊并生成结构化报告，供飞书 openClaw 机器人读取。 使用场景： 1. 导出指定微信群聊天记录（markdown/json）和成员列表 2. 分析群聊内容，输出时间线、组织架构、关键里程碑、活跃度等维度 3. 生成固定格式的记录文档（.md），放置于 open…"
---
...
| 项目 | 内容 |
|---|---|
| 来源 | [skills/nec-wechat-sync/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/nec-wechat-sync/SKILL.md) |
| 分类 | 核心推荐（仓库独家） |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/nec-wechat-sync/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
