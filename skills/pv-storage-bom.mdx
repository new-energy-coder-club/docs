---
title: "pv-storage-bom"
description: "Generate photovoltaic and energy storage system standard product BOM (Bill of Materials) lists in Excel format. Use when the user needs to create, or…"
---
...
| 项目 | 内容 |
|---|---|
| 来源 | [skills/pv-storage-bom/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/pv-storage-bom/SKILL.md) |
| 分类 | 核心推荐（仓库独家） |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/pv-storage-bom/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
