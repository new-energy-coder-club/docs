...
description: "| 切换 agent 的人设（soul.md）。支持三套预设人设与原有人设之间自由切换。 触发词：切换人设、persona-switch、赛博朋友、温柔伴侣、创始人龙虾。"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/persona-switch/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/persona-switch/SKILL.md) |
| 分类 | AI CLI 调用器 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/persona-switch/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
