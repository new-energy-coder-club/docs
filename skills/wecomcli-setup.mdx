---
title: "wecomcli-setup"
description: "企业微信 CLI 安装引导与自然语言翻译技能。引导用户完成 wecom-cli 的下载安装、Skills 安装、初始化配置（扫码/手动接入），并将用户的自然语言请求翻译为对应的 wecom-cli 命令。当用户说\"安装企业微信CLI\"、\"怎么用 wecom-cli\"、\"帮我配置企微机器人\"、\"初始…"
---

| 项目 | 内容 |
|---|---|
| 来源 | [skills/wecomcli-setup/SKILL.md](https://github.com/new-energy-coder-club/new_energy_coder_club/blob/Skill/skills/wecomcli-setup/SKILL.md) |
| 分类 | 集成 & 平台连接 |
| 安装 | 克隆 [Skill 分支](https://github.com/new-energy-coder-club/new_energy_coder_club/tree/Skill) 后，将 `skills/wecomcli-setup/` 目录复制到 `~/.claude/skills/`（Claude Code）、`~/.trae/skills/`（Trae IDE）或 `~/.qclaw/skills/`（QClaw/OpenClaw），详见 [Skill 安装方式](/community/skill-branch) |
